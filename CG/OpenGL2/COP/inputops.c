...
//	NO COMMON EDGES, edges are repeating
...
void addEdge(int v,edge *e){   // lists vertices  with incident edges
...
void addQuad(int index, int V1,int V2,int V3,int V4){             ///ITERATES THROUGH ALL FACES
	edge e1,e2,e3,e4;
...
void edgePointChange(i){
	
		printf("Changing Edge Point \n");
//		printf("x=%f, y=%f, z=%f \n", faces[i].e1.mid.x, faces[i].e1.mid.y, faces[i].e1.mid.z);
		faces[i].e1->mid.x+=.5*faces[i].facePt.x;
		faces[i].e1->mid.y+=.5*faces[i].facePt.y;
		faces[i].e1->mid.z+=.5*faces[i].facePt.z;

		faces[i].e2->mid.x+=.5*faces[i].facePt.x;
		faces[i].e2->mid.y+=.5*faces[i].facePt.y;
		faces[i].e2->mid.z+=.5*faces[i].facePt.z;

		faces[i].e3->mid.x+=.5*faces[i].facePt.x;
		faces[i].e3->mid.y+=.5*faces[i].facePt.y;
		faces[i].e3->mid.z+=.5*faces[i].facePt.z;

		faces[i].e4->mid.x+=.5*faces[i].facePt.x;
		faces[i].e4->mid.y+=.5*faces[i].facePt.y;
		faces[i].e4->mid.z+=.5*faces[i].facePt.z;

		printf("fpx+=%f, fpy+=%f, fpz+=%f \n", 0.5*faces[i].facePt.x, 0.5*faces[i].facePt.y, 0.5*faces[i].facePt.z);
//		printf("changedx+=%f, changedy+=%f, changedz+=%f \n", faces[i].e1.mid.x, faces[i].e1.mid.y, faces[i].e1.mid.z);
}
...
void input( void )
{
	static const char filename[] = "input4.txt";
