 ...
 * @return string
 */
	function day($fieldName, $minDay = null, $maxDay = null, $selected = null, $attributes = array(), $showEmpty = true) {
		...
		if (strlen($selected) > 2) {
			...
			$selected = null;
                ...
                $dayOptions = array('min' => $minDay, 'max' => $maxDay);
		return $this->select(
			$fieldName . ".day", $this->__generateOptions('day', $dayOptions), $selected, $attributes, $showEmpty
		);
	}
/**
 ...
 * @return string
 */
	function month($fieldName, $minMonth = null, $maxMonth = null, $selected = null, $attributes = array(), $showEmpty = true) {
		if ((empty($selected) || $selected === true) && $value = $this->value($fieldName)) {
			if (is_array($value)) {
				extract($value);
				$selected = $month;
				...
				if (empty($value)) {
					if (!$showEmpty && !$minMonth) {
                                            $selected = 'now';
					} elseif (!$showEmpty && $minMonth && !$selected) {
                                            $selected = $minMonth;
                                        }
				} else {
					$selected = $value;
		...
		unset($attributes['monthNames']);
                $monthOptions = array('min' => $minMonth, 'max' => $maxMonth, 'monthNames' => $monthNames);

		return $this->select(
			$fieldName . ".month",
			$this->__generateOptions('month', $monthOptions),
			$selected, $attributes, $showEmpty
		);
	}
/**
	...
	function dateTime($fieldName, $dateFormat = 'DMY', $timeFormat = '12', $selected = null, $attributes = array(), $showEmpty = true) {
		...
		$defaults = array(
			'minYear' => null, 'maxYear' => null,
                        ...
                        'separator' => '-', 'interval' => 1, 'monthNames' => true
		);
		$attributes = array_merge($defaults, (array) $attributes);
			...
			foreach (preg_split('//', $dateFormat, -1, PREG_SPLIT_NO_EMPTY) as $char) {
				switch ($char) {
					...
					case 'M':
						...
						$selects[] = $this->month($fieldName, $minMonth, $maxMonth, $month, $selectMonthAttr, $showEmpty);
					break;
					case 'D':
						$selects[] = $this->day($fieldName, $minDay, $maxDay, $day, $selectDayAttr, $showEmpty);
					break;
				}
			}
		...
		switch ($timeFormat) {
			case '24':
				...
				$opt .= '<br /><label>&nbsp;</label>' . $this->hour($fieldName, true, $hour, $selectHourAttr, $showEmpty) . ':' .
				$this->minute($fieldName, $min, $selectMinuteAttr, $showEmpty);
			break;
			case '12':
				$selectMinuteAttr['interval'] = $interval;
				$opt .= '<br /><label>&nbsp;</label>' . $this->hour($fieldName, false, $hour, $selectHourAttr, $showEmpty) . ':' .
				$this->minute($fieldName, $min, $selectMinuteAttr, $showEmpty) . ' ' .
