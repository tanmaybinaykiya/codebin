<?php
	...
	class UploadComponent extends Object {
		...
		function upload ($file, $destination, $name = NULL, $rules = NULL, $allowed = NULL) {
			...
			if (!is_null($allowed)) { $this->_allowed = $allowed; } else if (!is_null($rules)) { $this->_allowed = array('jpg','jpeg','gif','png'); }
			...
			if (isset($file) && is_array($file) && !$this->upload_error($file['error'])) {
				...
				if(!is_null($allowed) && !in_array($this->ext($fileName), $this->_allowed)){
		...
		function image ($file, $type, $size, $output = NULL, $quality = NULL) {
			...
			switch ($type) {
				...
				case 'resizewidth':
					# Maintains aspect ratio but resizes the image so that once
					# one side meets its maxW condition, it stays at that size
					# (thus one side will be larger)
					#get ratio
					$ratioX = $maxW / $uploadWidth;
			...
			$ext = substr($this->_name, strrpos($this->_name, '.'));
			// -- try to write
			switch ($output) {
				case 'jpg':
					$this->_name = str_replace($ext, ".jpg", $this->_name);
					$write = imagejpeg($dstImg, $this->_name, $quality);
					break;
				case 'png':
					imagesavealpha($dstImg, true);
					$this->_name = str_replace($ext, ".png", $this->_name);
					$write = imagepng($dstImg, $this->_name, $quality);
					break;
				case 'gif':
					$this->_name = str_replace($ext, ".gif", $this->_name);
